...
- **Track số / tên**: Track 8 — Bộ máy quiz & chấm tự động (Quiz & Auto Grading engine)
- **Big Ask — chép nguyên văn câu yêu cầu trong track card**:

```text
Hệ thống tự tạo quiz từ slide/instructor note/LMS và chấm formative để học viên biết mình hổng chỗ nào trước khi làm lab hoặc trước khi present.
```

- **Công cụ lớn này phục vụ ai** (học viên / coach / instructor / admin): Học viên (tự kiểm tra formative), Instructor & Coach (nắm bắt các hiểu lầm phổ biến để kịp thời can thiệp).
- **2 Red Flag đáng lo nhất (chép từ track card)**: 1. Quiz quá dễ, chỉ kiểm tra ghi nhớ · AI chấm sai câu trả lời mở.  2. Không phân biệt formative vs summative · không có nguồn từ tài liệu · học viên học mẹo trả lời quiz thay vì hiểu.
